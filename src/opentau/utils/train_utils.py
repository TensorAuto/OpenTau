...
def save_checkpoint(
    ...
    """accelerate saves the model and training run. This method saves all other axillary objects such as configs and rng state.
...
def prune_old_checkpoints(latest_checkpoint_path: str):
    """
    Recursively deletes all checkpoint directories in a parent folder except for the specified one.

    This function is designed to clean up old model checkpoints, preserving only the
    most recent one. It includes safety checks to ensure it only deletes directories
    and handles potential filesystem errors.

    Args:
        latest_checkpoint_path (str): The full path to the checkpoint
