...
class PI07HighLevelPlannerModel(nn.Module):
    ...
    Uses the Gemma 3 VLM backbone to encode images and a composite language
    ...
    Args:
        ...
        discrete_action_vocab_size: Vocabulary size for the discrete action
            tokenizer (passed through to ``Gemma3WithExpertModel``).
    """
    ...
    def forward(
        ...
        response_logits = self.gemma3_with_expert._lm_head()(response_out)
        ...
        memory_logits = self.gemma3_with_expert._lm_head()(memory_out)
    ...
    def infer_autoregressive(
        ...
        if auto_step == 0:
            ...
            token = prefix_out[:, -1:]
            token = self.gemma3_with_expert._lm_head()(token).argmax(dim=-1)
