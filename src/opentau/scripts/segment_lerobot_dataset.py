...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        Parsed CLI namespace containing input/output roots and a JSON path
...
def _load_segments_by_episode(segments_json: Path) -> dict[int, list[tuple[int, int]]]:
    """Load and validate segmentation plan from JSON.

    Args:
        segments_json: Path to JSON object mapping episode id strings to arrays
            of ``[start, end]`` pairs.

    Returns:
        Mapping from integer episode id to list of ``(start, end)`` tuples.

    Raises:
        ValueError: If JSON content is missing or malformed.
    """
    if not segments_json.is_file():
        raise ValueError(f"Segments JSON file does not exist: {segments_json}")

    with open(segments_json) as f:
        data = json.load(f)

    if not isinstance(data, dict):
        raise ValueError("Segments JSON must be an object mapping episode ids to segment lists.")

    segments_by_episode: dict[int, list[tuple[int, int]]] = {}
    for episode_key, segment_list in data.items():
        if not isinstance(episode_key, str):
            raise ValueError("Episode ids in segments JSON must be strings.")
        try:
            episode_id = int(episode_key)
        except ValueError as exc:
            raise ValueError(f"Episode id '{episode_key}' is not a valid integer string.") from exc

        if not isinstance(segment_list, list) or len(segment_list) == 0:
            raise ValueError(f"Episode '{episode_key}' must map to a non-empty list of [start, end] pairs.")

        validated_segments: list[tuple[int, int]] = []
        for seg_idx, pair in enumerate(segment_list):
            if not isinstance(pair, (list, tuple)) or len(pair) != 2:
                raise ValueError(
                    f"Segment #{seg_idx} for episode '{episode_key}' must be a pair [start, end]."
                )
            start_raw, end_raw = pair
            if not isinstance(start_raw, int) or isinstance(start_raw, bool):
                raise ValueError(
                    f"Segment start must be an integer for episode '{episode_key}', got {start_raw}."
                )
            if not isinstance(end_raw, int) or isinstance(end_raw, bool):
                raise ValueError(
                    f"Segment end must be an integer for episode '{episode_key}', got {end_raw}."
                )
            start, end = int(start_raw), int(end_raw)
            if start < 0 or end < 0:
                raise ValueError(
                    f"Segment [{start}, {end}] for episode '{episode_key}' must be non-negative."
                )
            if end <= start:
                raise ValueError(
                    f"Segment [{start}, {end}] for episode '{episode_key}' must satisfy end > start."
                )
            validated_segments.append((start, end))

        segments_by_episode[episode_id] = validated_segments

    if not segments_by_episode:
        raise ValueError("Segments JSON produced an empty segmentation plan.")
    ...
    for frame_index, cell in enumerate(image_cells):
        ...
        if isinstance(cell, dict):
            ...
            if isinstance(image_bytes, (bytes, bytearray)) and len(image_bytes) > 0:
                ...
                new_cell = dict(cell)
                # Avoid duplicating image payload in parquet after materializing to disk.
                new_cell["bytes"] = None
...
def segment_dataset(
    ...
    if not segments_by_episode:
        raise ValueError("At least one episode with segment ranges must be provided.")
    ...
    for source_episode_id, episode_segments in segments_by_episode.items():
        if source_episode_id not in source_meta.episodes:
            raise ValueError(f"Episode {source_episode_id} not found in source dataset.")
        if not episode_segments:
            raise ValueError(f"Episode {source_episode_id} has no segment ranges.")
        source_length = int(source_meta.episodes[source_episode_id]["length"])
        for start, end in episode_segments:
            if not (0 <= start < end <= source_length):
                raise ValueError(
                    f"Segment ({start}, {end}) is invalid for source episode length {source_length}. "
                    "Expected 0 <= start < end <= source_length."
                )
...
def main() -> None:
    """CLI entry point."""
    args = parse_args()
    segments_by_episode = _load_segments_by_episode(args.segments_json)
    segment_dataset(
