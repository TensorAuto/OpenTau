...
def make_teleoperator_from_config(config: TeleoperatorConfig) -> "Teleoperator":
    """SO-100/SO-101-only teleoperator factory (trimmed from upstream LeRobot)."""
    ...
    raise ValueError(
        f"Teleoperator type '{config.type}' is not supported by the vendored SO-101 stack. "
        "Supported: so100_leader, so101_leader, bi_so_leader."
    )
