...
from opentau.scripts.so101.constants import HF_LEROBOT_CALIBRATION, TELEOPERATORS
from opentau.scripts.so101.motors.motors_bus import MotorCalibration
from opentau.scripts.so101.types import RobotAction
...
class Teleoperator(abc.ABC):
    ...
    def _save_calibration(self, fpath: Path | None = None) -> None:
        ...
        """
        fpath = self.calibration_fpath if fpath is None else fpath
        # draccus.dump can't encode MotorCalibration here: motors_bus uses
        # `from __future__ import annotations`, so its fields are string annotations
        # ('int') and draccus 0.8.0's encoder weakref-caches that str and crashes.
        # dataclasses.asdict + json produces the identical on-disk format.
        payload = {name: dataclasses.asdict(cal) for name, cal in self.calibration.items()}
