...
# Add the dummy/repo mapping to DATA_FEATURES_NAME_MAPPING for pytesting
...
def img_tensor_factory():
    def _create_img_tensor(height=100, width=100, channels=3) -> torch.Tensor:
...
def lerobot_dataset_metadata():
    ...
    mock_metadata.root = Path("/tmp/mock_root")
    mock_metadata.info = {
        "codebase_version": CODEBASE_VERSION,
        ...
        "video_path": "videos/",
        "features": {
            "state": {"dtype": "float32", "shape": [32]},
            "actions": {"dtype": "float32", "shape": [50, 32]},
            "camera0": {"dtype": "video", "shape": [3, 224, 224], "fps": 30, "duration": 5.0},
        },
    }
    mock_metadata.stats = {
        "state": {
            "max": np.array([1.0] * 32, dtype=np.float32),
            "mean": np.array([0.5] * 32, dtype=np.float32),
            "min": np.array([0.0] * 32, dtype=np.float32),
            "std": np.array([0.25] * 32, dtype=np.float32),
            "count": np.array([150], dtype=np.int64),
        },
        "actions": {
            "max": np.full((50, 32), 1.0, dtype=np.float32),
            "mean": np.full((50, 32), 0.5, dtype=np.float32),
            "min": np.full((50, 32), 0.0, dtype=np.float32),
            "std": np.full((50, 32), 0.25, dtype=np.float32),
            "count": np.array([150], dtype=np.int64),
        },
        "camera0": {
    ...
    mock_metadata.episodes_stats = {
        0: {"episode_index": 0, "stats": mock_metadata.stats},
    ...
    mock_metadata.episodes = {
        ...
        1: {"episode_index": 1, "tasks": ["Perform action 0."], "length": 50},
        2: {"episode_index": 2, "tasks": ["Perform action 0."], "length": 50},
    }
    ...
    def _create_lerobot_dataset(
        ...
        **kwargs,
        ...
        with (
            ...
            return LeRobotDataset(cfg=cfg, repo_id=repo_id, root=root, **kwargs)
...
def empty_lerobot_dataset_factory() -> LeRobotDatasetFactory:
    return partial(LeRobotDataset.create, repo_id=DUMMY_REPO_ID, fps=DEFAULT_FPS, standardize=False)


@pytest.fixture
def lerobot_dataset(lerobot_dataset_factory, tmp_path):
    """Create a simple LeRobot dataset for testing."""
...
@pytest.fixture
def datasets_factory(lerobot_dataset_factory, lerobot_dataset_metadata_factory, tmp_path):
    def _create_datasets(num_datasets: int = 5):
        datasets = [
            lerobot_dataset_factory(
                root=tmp_path / f"dataset_{i}",
                repo_id=DUMMY_REPO_ID,
                total_episodes=3,
                total_frames=150,
                total_tasks=1,
