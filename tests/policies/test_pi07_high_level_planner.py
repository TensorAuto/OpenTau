...
NUM_CAMERAS = 2
# Gemma 3's `Gemma3MultiModalProjector` reduces every image to
# `mm_tokens_per_image` tokens regardless of the underlying patch grid, so each
# camera contributes 256 tokens to the prefix even though the SigLIP backbone
# produces a 32x32=1024 patch grid at 448x448.
...
class TestPI07HighLevelPlannerIntegration:
    ...
    def _indicator_lens(tokenizer):
        ...
        The Gemma 3 tokenizer (``google/gemma-3-4b-pt``) may tokenize these
        spans differently than PaliGemma did. We resolve against the live
        tokenizer so the prefix-layout assertions track the real token count.
        """
        return {
    ...
    def test_complete_pi07_high_level_planner_pipeline(self, lerobot_dataset_metadata):
        ...
        batch_size = 1
        batch = {
            "camera0": torch.randn(batch_size, 3, IMAGE_SIZE, IMAGE_SIZE),
            "camera1": torch.randn(batch_size, 3, IMAGE_SIZE, IMAGE_SIZE),
