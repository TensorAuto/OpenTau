...
def test_constant_scheduler(optimizer):
    config = ConstantSchedulerConfig(num_warmup_steps=0)
    scheduler = config.build(optimizer, num_training_steps=100)
    assert isinstance(scheduler, LambdaLR)

    # Test that learning rate remains constant over multiple steps
    initial_lr = optimizer.param_groups[0]["lr"]

    optimizer.step()
    scheduler.step()
    lr_after_first_step = optimizer.param_groups[0]["lr"]
    assert lr_after_first_step == initial_lr

    # Take a few more steps to ensure it stays constant
    for _ in range(5):
        optimizer.step()
        scheduler.step()
