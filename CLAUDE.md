...
8. **Freeze-flag trainable sets are guarded by set-equality + full-set gradient-reachability tests — graph-shape assertions guard nothing.** When adding or extending a mode that partitions parameters into trainable/frozen (`train_expert_only`, `train_state_action_representation_only`, …), start from the default-deny, module-object-resolved helpers in `policies/utils.py` (`freeze_*_for_state_action_representation_only`; their docstrings explain why parameter-name matching and enumerated freezing are unsafe — `Normalize`/`Unnormalize` register stats as `nn.Parameter(requires_grad=False)` named `buffer_state`/`buffer_actions`, and the optimizer factory selects on `requires_grad` alone). Both of the following were review findings on #497:
   - **A "grad reaches only the trainable set" test must run one backward and assert `grad is not None` on *every* intended-trainable parameter — not a spot-checked subset — and must construct every wrapper flavour that declares the flag.** A severed path (e.g. a stray `.detach()` upstream of the one projection that routes through `embed_suffix` instead of straight to the output) passes a subset assertion while part of the trainable set silently never trains; #497's test shipped asserting 2 of 3 projections, and separately never constructed the Qwen3-VL wrapper at all — the flavour the tests skip is where the gap lives. Pair it with an observed-trainable-set equality test and an AST completeness test so a later policy can't dodge the sweep (patterns: `tests/policies/test_state_action_representation_only_policies.py`, `..._registry.py`).
   - **`grad_fn is None` / "no graph retained" assertions are vacuous as guards on a `torch.no_grad()` predicate around a fully-frozen module.** When no leaf requires grad, autograd builds no graph and retains no activations either way, so the assertion passes under both polarities of the predicate. Such a `no_grad` (cosmos3 `run_prefix`) is belt-and-braces self-documentation — a truthful answer to "is any backbone parameter trainable?" — not a memory optimization, and neither tests nor docstrings should claim otherwise. Mutation-check any new regression test before presenting it as a guard: revert the change it supposedly pins and confirm the test fails; if it still passes, re-docstring it as pinning an observable contract and point at the trainable-set/reachability tests as the real guards.
