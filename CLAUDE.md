...
6. **Per-rank GPU pinning through an inherited env var must index *through* `CUDA_VISIBLE_DEVICES`, not write the raw local rank.** When you assign a per-rank render/compute device by setting an environment variable that a child process or vec-env worker will inherit (e.g. `MUJOCO_EGL_DEVICE_ID` for MuJoCo's EGL backend), use the local rank as an index *into* `CUDA_VISIBLE_DEVICES` (`visible[local_process_index]`), falling back to the raw `local_process_index` only when `CUDA_VISIBLE_DEVICES` is unset. A masked or reordered visible subset (e.g. `CUDA_VISIBLE_DEVICES="4,5,6,7"`) means rank 1's physical GPU is `5`, not `1` — writing the raw index renders on the wrong GPU (or piles every rank onto device 0) and trips robosuite's `MUJOCO_EGL_DEVICE_ID in CUDA_VISIBLE_DEVICES` assert. Set it *before* the env is built, since `AsyncVectorEnv` spawn workers snapshot `os.environ` at spawn. Copy `_pin_egl_render_device` in `src/opentau/envs/factory.py`, don't reinvent it. Like the patterns in rule 5, this is silent at CPU smoke-test time and only manifests on real multi-GPU hardware.
