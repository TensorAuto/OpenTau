...
5. **Distributed forward/backward must keep collective counts aligned across ranks.** FSDP / ZeRO-3 hang at NCCL with mismatched all-gather sizes when ranks disagree on what `forward` does. The failure mode is silent at CPU smoke-test time and only manifests after collectives diverge on a real run, so the patterns below are mandatory — copy them when extending the relevant code, don't reinvent them:
   ...
   - **Gate a count-driven collective with `accelerator.reduce(..., reduction="sum")`, not `gather_for_metrics`.** When a per-step collective (e.g. a `gather_object`) only needs to fire on the rare steps that carry data, gate it on a globally-summed count so every rank takes the same branch — see `log_outlier_records_distributed` in `src/opentau/scripts/train.py`. Compute that count with `accelerator.reduce(..., reduction="sum")`: it returns an identical value to every rank, so the gate is rank-aligned. `gather_for_metrics` is the wrong primitive for the gate — it trims dataloader-padding samples at end-of-epoch, which can shave the count and silently drop the gated work on the last batch. Don't "consistency-fix" such a gate back to `gather_for_metrics` to match surrounding metric gathers (which legitimately use it because they *want* the padding trim).
