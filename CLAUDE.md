...
4. **When deprecating a draccus config field that already has a non-`None` default, switch the default to a `None` sentinel and test the negative case.** Triggering a `DeprecationWarning` from `__post_init__` by comparing against a sibling field (e.g. `if dataset_cfg.val_split_ratio != self.val_split_ratio: warn(...)`) cannot distinguish "user never set this" from "user set it to the previous default" when the field still defaults to a real value — so the warning fires for every user who only customizes the new field. Use `field: T | None = None` and warn only when the value is not `None`. Tests must include a *negative* assertion that no warning fires in normal use (only the new field set); a positive-only test will pass against the buggy implementation. See `src/opentau/configs/default.py::DatasetConfig.val_split_ratio` and `tests/configs/test_default.py::test_val_split_ratio_no_warning_when_only_mixture_customized` for the canonical shape.
