...
6. **Feature-name mapping resolution must prefer the per-instance/per-entry mapping; the process-global registry is a fallback, not the source of truth.** `DATA_FEATURES_NAME_MAPPING` (`src/opentau/datasets/standard_data_format_mapping.py`) is last-wins and keyed by `repo_id[::control_mode]`, so two mixture entries sharing a `repo_id` and `control_mode` but declaring different `data_features_name_mapping` values — a legitimate config, e.g. two camera views of the same repo — clobber each other in the registry. Pre-#458 this silently trained two identical copies of one view with plausible loss curves. Per-instance mappings are now threaded end-to-end; any new code resolving feature names must follow the same pattern, not read the registry directly:
   - **Fetch-time (inside `BaseDataset` subclasses):** resolve via `self._get_name_map()` (`src/opentau/datasets/lerobot_dataset.py`) — instance mapping first, registry fallback. Never index `DATA_FEATURES_NAME_MAPPING` directly from dataset methods.
   - **Config-/setup-time (code holding a `DatasetConfig`):** prefer `dataset_cfg.data_features_name_mapping` when set, registry as fallback — like `resolve_delta_timestamps` in `src/opentau/datasets/factory.py` and the per-entry `name_maps` param of `DatasetMixtureMetadata` (`src/opentau/datasets/dataset_mixture.py`).
   - **Threading new per-entry state through the mixture:** account for wrapper layers — `random_split` `Subset` wrappers need the one-level-deep unwrap (see the `name_maps` threading in `WeightedDatasetMixture`), and val-split clones must share the instance mapping.
   - Direct registry reads are legitimate only for consumers with no config entry in hand (annotation scripts, `envs/subgoal.py`, `fit_fast_tokenizer.py`). #458's review caught the same miss three times in one PR (`resolve_delta_timestamps`, mixture normalization stats, `diagnose_norm_distribution.py`) — when touching mapping resolution, audit *every* consumer on your change's path, including diagnostic scripts that claim to mirror training.
