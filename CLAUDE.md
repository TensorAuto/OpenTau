...
5. **Distributed forward/backward must keep collective counts aligned across ranks.** FSDP / ZeRO-3 hang at NCCL with mismatched all-gather sizes when ranks disagree on what `forward` does. The failure mode is silent at CPU smoke-test time and only manifests after collectives diverge on a real run, so the patterns below are mandatory — copy them when extending the relevant code, don't reinvent them:
   ...
   - **Per-sample tensors that must stay row-aligned across ranks belong in one `gather_for_metrics({...})` dict call.** `accelerate.gather_for_metrics` de-pads a ragged final batch (when total samples aren't divisible by `world_size`); issuing a separate call per tensor relies on each independent de-pad landing on the exact same trim, which silently desyncs paired rows (e.g. per-sample losses vs. their `dataset_index` / source-index provenance). Gather all the paired per-sample tensors together in a single dict call so one identical trim applies to every entry — like the validation per-sample gather in `src/opentau/scripts/train.py`. This is a correctness bug, not a hang, and the CPU suite can't catch it (the multi-rank de-pad path only fires on >1-rank hardware).
