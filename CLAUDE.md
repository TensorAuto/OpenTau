...
7. **A flag that routes gradients to a parameter subset must be checked against knowledge insulation's detach, and its name-based contracts pinned by tests.** Both patterns below were reviewer catches on #488 (`train_vision_encoder_only`); sweep for them when adding or rewiring a training-mode flag (`freeze_vision_encoder` / `train_expert_only` / `train_vision_encoder_only` or a new sibling):
   - **Knowledge insulation severs action-loss gradients to the prefix.** With `knowledge_insulation=True` (the default), the prefix KV cache is detached before the action expert (`policies/pi05/modeling_pi05.py`), so the flow-matching (MSE) loss cannot reach the vision tower or LLM backbone. A flag that expects action-loss gradients on prefix params silently trains on CE alone — and with CE loss weight 0, the step has no gradient path at all. Cross-validate the interaction in `__post_init__` on **all five** configs that define `knowledge_insulation` (pi05, pi05_mem, pi06, pi07 low-level, pi07_paligemma low-level) — warn rather than error, since CE-only prefix training is a legitimate partial signal — and pin the warning with `caplog` tests on both exposure paths: the direct policy-level field and pi07's `vlm_config` indirection. (pi07's top-level freeze fields are vestigial/unplumbed; its training-mode flags live on `vlm_config`, e.g. `--policy.vlm_config.train_vision_encoder_only=true`, so its validation must read `self.vlm_config.<flag>`.)
   - **Behavior keyed off a module attribute name needs a real-module integration test.** `freeze_policy_level_params_for_vision_only` (`policies/utils.py`) keeps params trainable when their qualified name contains `motion_module`; renaming that attribute in `RLDXVideoEncoder` would silently freeze the temporal block with nothing failing. When code keys off a name string, add a test that builds the real module and asserts the contract — like `test_freeze_vision_only_keeps_real_rldx_motion_module_trainable` in `tests/policies/test_rldx_motion_cpu.py` — so a rename breaks a test instead of a training run.
...
`config_version` (`configs/policies.py::CURRENT_CONFIG_VERSION`) is a monotonic schema version for **behavioral conventions** the weights were trained under — NOT the package version. Bump it only when a change alters what the model sees/produces and must be gated so old checkpoints keep their trained behavior (v1 = zero-range MIN_MAX/QUANTILE dims map to `0.0` not `-1.0`; see `CHANGELOG.md`). The field is `int | None`; `None` behaves as CURRENT for behavior but is resolved to a concrete int automatically — to `0` (legacy) when `PreTrainedPolicy.from_pretrained` loads pre-tag weights (it peeks the source's `config.json`/`train_config.json`), and to CURRENT for a fresh/scratch run (`factory.make_policy` + save concretization). **Never resolve it in `__post_init__`** — that erases the "was the key absent?" signal the loader needs. A gated convention read at forward time must reach `Normalize`/`Unnormalize` as a plain-bool kwarg (`zero_range_center`, default legacy `False`) computed once from `config.zero_range_centers_on_zero()`, so it is identical across ranks and constant-folds under compile (CLAUDE.md rule 5). Fitted FAST tokenizers version-stamp the same convention (`opentau_action_norm.json` sidecar) and a policy raises on mismatch at load.
