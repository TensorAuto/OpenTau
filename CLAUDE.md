...
6. **Normalization stat names must flow through `stat_names_for_mode` — never hardcode stat-key tuples.** Each `NormalizationMode` reads different stat buffers (`MEAN_STD` → `mean`/`std`, `MIN_MAX` → `min`/`max`, `QUANTILE` → `q01`/`q99` with per-row min/max fallback for pre-quantile datasets); `stat_names_for_mode` in `src/opentau/policies/normalize.py` is the single source of truth. Code that enumerates stat names inline breaks *silently* when a mode is added — #478's blocking bug was exactly this: the head-count probe in `pretrained.py` searched a hardcoded `("mean", "std", "min", "max")`, found nothing in QUANTILE-only buffers, fell back to 1 head, and silently normalized every unidentified inference sample with dataset 0's stats instead of raising. Three more findings in the same PR were the same pattern in other files. When touching normalization modes or stat keys: (a) resolve stat names via `stat_names_for_mode` (and its `_QUANTILE_FALLBACK` resolution), never a literal tuple; (b) keep structural code shape-based, not name-based — e.g. `_stacked_num_datasets` now takes the leading dim of *any* buffer entry, since every stat shares the `(num_datasets, ...)` leading dim by construction; (c) before finishing, grep the codebase for leftover hardcoded stat-name enumerations — `datasets/dataset_mixture.py` (padding + non-finite sweeps) and `scripts/diagnose_norm_distribution.py` legitimately enumerate stat keys and must be extended in the same change.
