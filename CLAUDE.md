...
3. **Verify determinism on any change to the training loop or model.** ML bugs hide in stochasticity: a bad change can still produce loss curves that *look* plausible. After touching anything in `scripts/train.py`, `policies/*/modeling_*.py`, `optim/`, or `datasets/sampler.py`, run a smoke config twice with the same `seed` and confirm the per-step loss series is bit-identical (not just "close"). Seeding utilities live in `src/opentau/utils/random_utils.py` (`set_seed`, `serialize_python_rng_state`, etc.). If two seeded runs diverge, that's a bug — investigate before claiming the change works. Known exception (#470): GPU runs at a native (non-config) input resolution with an *unfrozen* vision tower (`freeze_vision_encoder=False`) are legitimately not bit-reproducible — position-embedding interpolation backprops through CUDA's bicubic `F.interpolate` backward, which is atomicAdd-nondeterministic (documented at both interpolation sites: `utils/transformers_patch.py::get_image_features` and `policies/pi07/video_encoder.py`). Run the seeded-twice check on CPU (`accelerate launch --cpu`) or at the config resolution / default frozen tower, where it stays bit-exact. If a change introduces another op on torch's nondeterministic-ops list, document the exact reproducibility boundary at the site the same way — a narrow, documented exception, never a blanket "GPU is nondeterministic" shrug.
...
6. **Per-policy helpers are deliberately duplicated — fix every copy; put *new* cross-policy logic in one shared, unit-tested helper.** Preprocessing helpers like `resize_with_pad` exist as near-identical copies in every `policies/*/modeling_*.py` plus `datasets/lerobot_dataset.py` and `envs/subgoal.py` (11 copies as of #470). A bug fix to one copy is incomplete until you've grepped for the function name and patched all of them — the same bug otherwise survives in every policy you didn't touch. Conversely, logic *newly* needed by several policies (validation guards, geometry checks) should be written once and unit-tested, not pasted per policy — e.g. `assert_gemma3_input_resolution` in `src/opentau/policies/utils.py` and `validate_eval_input_resolution` in `scripts/eval.py`, both extracted from triplicated inline guards during #470 review. Related argument-order gotcha from the same PR: config `resolution` tuples are `(height, width)` but `resize_with_pad(img, width, height, ...)` takes width first — never splat a resolution tuple into its positional args; pass explicit `width=` / `height=` keywords (the splatted form silently transposed the target at every call site, invisible at square defaults).
