...
**A `DecodingError` from `--config_path` on an old published checkpoint can be by design — never "fix" a *renamed* field by adding it to `_STRIPPED_FIELDS`.** `--policy.path` decodes only the `.policy` sub-object of `hf_config.json` and works on old checkpoints, but `--config_path` decodes the whole `TrainPipelineConfig`, so a train config that predates a schema change raises before the policy is reached (e.g. `grounding`, renamed to `vqa` in #124). The `_STRIPPED_FIELDS` machinery (`configs/policies.py`) is only for **removed** fields, where dropping the key is lossless; stripping a renamed field would let the config decode with the new field at its default and silently lose a setting the run was actually trained with — strictly worse than failing loudly (#494). Point users at `--policy.path` for old checkpoints, or hand-edit the fetched config.
