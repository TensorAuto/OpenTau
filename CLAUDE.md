...
6. **Dataset sample-schema and episode-metadata invariants.** Two silent failure modes in `src/opentau/datasets/` that unit tests on isolated resolvers pass right through (each cost a blocking review round in #457):
   - **Every dataset in a mixture must emit an identical per-sample schema.** `WeightedDatasetMixture`'s DataLoaders (`datasets/dataset_mixture.py`) pass no `collate_fn`, so default collate keys off `batch[0]` — a key emitted by only some datasets in a heterogeneous mixture is an order-dependent `KeyError` (or a silent drop) depending on shuffle order. New optional per-sample keys must be emitted uniformly for every dataset via `_emit_optional_keys` in `datasets/lerobot_dataset.py` (padded when the source column is absent), and role entries in `data_features_name_mapping` (e.g. `mistake`/`success`) must be skipped in `_to_standard_data_format`'s generic copy loop so raw source columns never leak into the sample schema. When adding a per-sample key, test through `_to_standard_data_format` + `default_collate` across a mapped *and* an unmapped dataset — testing the resolver in isolation misses this entire bug class.
   - **Per-episode stats live only in `meta.episodes_stats`, never as flat `stats/*` keys on `meta.episodes`.** `load_episodes_v30` (`datasets/utils.py`) strips every `stats/*` column from episode records; the aggregates are reconstructed into the nested `meta.episodes_stats[ep_idx][col]["min"/"max"/"mean"]` dict. Code reading `meta.episodes[ep].get("stats/<col>/...")` is dead on every real dataset. Gate per-episode-aggregate logic on codebase version ≥ v2.1 — the v2.0 backward-compat path replicates the *global* aggregate into every episode, which misreads as per-episode data. Build test fixtures for metadata-shape-dependent code via the real loaders (`load_episodes_v30` / `load_episodes_stats_v30` over an in-memory Arrow table), not hand-built dicts: hand-built flat-key fixtures passed twice in #457 while the code under test was dead on real data.
