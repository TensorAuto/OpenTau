...
4. **Isolate construction-time RNG draws.** Any code path inside a model `__init__` (or a helper called from it) that consumes the global torch RNG — `model.resize_token_embeddings`, a fresh `nn.init.*` on a new sub-module, `torch.randn(...)` for a buffer — must wrap the draw in `torch.random.fork_rng(devices=...)` and re-seed inside with a fixed constant. Otherwise the new tensor values couple to wherever in the build sequence the helper happens to fire, and any later refactor that reorders construction (or adds a new call site) silently violates rule #3 above without changing any user-visible config. See `src/opentau/datasets/grounding/tokenizer_utils.py::ensure_loc_tokens` (`_LOC_EMBEDDING_INIT_SEED` + `fork_rng` snapshot/restore over CPU + visible CUDA devices) for the canonical shape.
