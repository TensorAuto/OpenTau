...
- **PRs must follow `.github/PULL_REQUEST_TEMPLATE.md` verbatim.** The `check-pr-checklist.yml` CI grep-matches the exact phrases for the docstring and policy-change checkboxes — do not rephrase them, do not delete sections. Fill in `What this does`, `How it was tested`, and `How to checkout & try`. Create the PR with the full template body from the start (`gh pr create --body-file ...`, never `--fill`/`--fill-first`): the workflow reads `context.payload.pull_request.body` from the *event payload*, not the live PR, so if a template-less body slips in, fixing the body and re-running the failed job just replays the stale payload — only a fresh `edited`/`synchronize` event (a body edit or a push) re-evaluates it (#492).
