...
- **Don't suggest "release notes" or changelog entries for behavior/default-value changes.** OpenTau does not maintain a CHANGELOG; the PR description and commit message are the only places such changes are recorded. Reviews that flag a silent default change as needing a release note will be rejected — note the change in the PR description instead.
