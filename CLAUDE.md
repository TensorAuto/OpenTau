...
**`@parser.wrap()` must sit directly on the entry-point `def` (`train`, `eval_main`).** A decorator binds to the function immediately below it, so inserting a helper between `@parser.wrap()` and the entry `def` silently re-wraps the helper instead — the entry point is then called unwrapped and raises `TypeError: ... missing 1 required positional argument: 'cfg'`, breaking the whole console script (`opentau-train` / `opentau-eval`). This is silent at CPU-test time: the suite exercises `eval_policy`/helpers directly and never the wrapped entry. When adding helpers near a wrapped entry point, define them *above* the decorator, and pin placement with a `__wrapped__` assertion (see `TestEvalUsesShardedParams.test_eval_main_is_parser_wrapped` in `tests/scripts/test_eval.py`).
