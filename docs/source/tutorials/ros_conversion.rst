...
The ROS to LeRobot conversion script is located in ``src/opentau/scripts/convert_ros_to_lerobot.py`` which takes a config file as input. Example config is mentions in ``configs/examples/ros2lerobot.json``.

The example cnofig looks like:
.. code-block:: javascript
    ...
    "dataset_features": {
        <feature_name in leroobt dataset>: {
