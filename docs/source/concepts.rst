...
Policies are implemented as PyTorch modules and inherit from ``lerobot.common.policies.pretrained.PreTrainedPolicy``.
...
The main configuration class is ``lerobot.configs.train.TrainPipelineConfig``, which orchestrates training settings,
...
Currently, only `Libero <https://libero-project.github.io/main.html>`_ is supported and it is configured via ``lerobot.common.envs.configs.LiberoEnv``.
